...
export class PrismaUserRepository implements UserRepository {
  ...
  async findByRecoverKey(recoverKey: string): Promise<UserEntity | null> {
    const user = await this.prisma.user.findFirst({
      where: { recoverKey },
    });
  ...
  async findByKeepAlive(keepAlive: string): Promise<UserEntity | null> {
    const user = await this.prisma.user.findFirst({
      where: { keepAlive },
    });
  ...
  async findByCommission(commission: string): Promise<UserEntity | null> {
    const user = await this.prisma.user.findFirst({
      where: { commission },
    });
